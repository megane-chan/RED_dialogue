...
```{r}
# read excel
dialog_data <- read_excel("", sheet = "Sheet1_Dialogs") %>% select(-receiver_dialog)
head(dialog_data)
```
...
high_perf_all %>% select(-c(sender, receiver)) %>% select(session, time, sender_id, receiver_id, everything()) -> high_perf_all
low_perf_all %>% select(-c(sender, receiver)) %>% select(session, time, sender_id, receiver_id, everything()) -> low_perf_all
```

```{r}
# save high and low performance data
saveRDS(high_perf_all, "data/high_final.RData")
saveRDS(low_perf_all, "data/low_final.RData")
```

```{r}
```

### Extract High and Low Performing Sessions
