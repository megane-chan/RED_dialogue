...
```{r}
# find find when sender id in actor attribtue
all_sessions %>% filter(sender_id %in% actors_attributes$id) %>% filter(receiver_id %in% actors_attributes$id) 

session_dialogues <- all_sessions %>%
  group_by(session, performance) %>%
...
ggplot(session_dialogues, aes(x = session, y = n, fill = factor(performance))) + 
  ...
  geom_text(aes(label = session), y = -50, vjust = .5, angle = 45, size = 3) +
  ...
  labs(subtitle = "Number of Dialogues By Session",
       x = "",
       y = "Number of Dialogues",
       fill = "Performance") 

ggsave("image/dialogues_by_session.png", width = 8, height = 6)
```
...
```{r}
# take away 2103
session_dialogues %>% filter(session != 2103) -> session_dialogues
ggplot(session_dialogues, aes(x = session, y = n, fill = factor(performance))) + 
  geom_bar(stat = "identity") +
  scale_fill_brewer(palette = "Pastel1") +  
  geom_text(aes(label = n), vjust = -0.5, size = 3) +
...
# plot
ggplot(dialogues_per_speaker, aes(x = name, y = number_of_dialogues, fill = name)) + 
  geom_bar(stat = "identity") +
  scale_fill_brewer(palette = "Pastel1") +  
...
ggplot(dialogues_per_dialogue, aes(x = dialog, y = number_of_dialogues, fill = as.factor(dialog))) + 
  geom_bar(stat = "identity") +
  scale_fill_brewer(palette = "Pastel1") +  
  labs(caption = "Number of Dialogues By Dialogue Type (Across All Sessions)",
       x = "Dialogue Type",
       y = "Number of Dialogues") +  
   geom_text(aes(label = dialog), y = -45, vjust = -.5, angle = 45, size = 3) +
 theme_graph(base_family = "Times", caption_size = 12, base_size = 11, plot_margin = margin(4,2,6,2)) +
...
```{r}
# pLOT LOG SINCE STATEMENT IS TOO MUCH
log_dialogues <-log(dialogues_per_dialogue$number_of_dialogues)

ggplot(dialogues_per_dialogue, aes(x = dialog, y = log(number_of_dialogues), fill = dialog)) + 
  # bar is adjusted log scaled
  geom_bar(stat = "identity") +
  scale_fill_brewer(palette = "Pastel1") +  
  labs(caption = "Number of Dialogues By Dialogue Type",
       x = "",
       y = "Number of Dialogues",
       legend = "Dialogue Type") +
  geom_text(aes(label = dialog), y = -45, vjust = -.5, angle = 45, size = 5) +
 theme_graph(base_family = "Times", caption_size = 12, base_size = 12, plot_margin = margin(2,1,2,1)) +
  geom_text(aes(label = number_of_dialogues), vjust = -0.5, size = 3) +
  theme(legend.position = "right", legend.title = element_text(size = 12))# content of legend
...
ggsave("image/dialogues_per_speaker_session_all.png", width = 8, height = 6)
```
...
all_sessions %>% filter(receiver_id != 0) %>% select(-session) %>% mutate(time = 1:nrow(.))  %>% select(sender_id, receiver_id, time,dialog)  %>% mutate(dialog = as.factor(dialog), sender_id = as.integer(sender_id), receiver_id = as.integer(receiver_id)) -> interactions
...
for (s in unique(all_sessions$session)) {
  session_s %>% filter(session == s) %>% 
    filter(sender_id %in% actors_attributes$id) %>% 
    filter(receiver_id %in% actors_attributes$id)  %>% graph_from_data_frame(vertices = data.frame(actors_attributes)) %>% ggraph(layout = 'fr') 
}
```
...
ggraph(g_subset, layout = 'fr') +
  ...
  theme_void() +
  labs(subtitle = "", color = "Gender", edge_color = "Dialog") +
  theme(legend.position = "right", legend.title = element_text(size = 8))

#Savage("image/dialogue_flow_all.png", width = 8, height = 6)
```
